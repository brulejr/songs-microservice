...
class SongController(val songService: SongService) {
    ...
    fun createTask(@RequestBody song: Song): Mono<EntityModel<Song>> {
    ...
    fun deleteTask(@PathVariable songGuid: UUID): Mono<Void> {
        return songService.delete(songGuid)
    }

    @GetMapping("/{songGuid}")
    fun getTaskById(@PathVariable songGuid: UUID): Mono<EntityModel<Song>> {
    ...
    @GetMapping
    fun listTasks(): Flux<EntityModel<Song>> {
    ...
    fun patchTask(@PathVariable songGuid: UUID, @RequestBody taskPatch: JsonPatch): Mono<EntityModel<Song>> {
